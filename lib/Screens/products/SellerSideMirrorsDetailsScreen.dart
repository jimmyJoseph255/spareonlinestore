...
class _SellerSideMirrorsDetailsScreenState
    ...
    try {
      ...
      setState(() {
        _products = products.map<Map<String, dynamic>>((product) {
          String imageUrl = _processImageData(product['image']);
          return {
            'name': product['compatibility']?.toString() ?? 'No Name',
  ...
  String _processImageData(dynamic imageData) {
    String imageUrl = '';
  ...
  String _constructImageUrl(String imagePath) {
    ...
    return 'https://sparefasta.co.tz/storage/${imagePath.startsWith('/') ? imagePath.substring(1) : imagePath}';
  }
