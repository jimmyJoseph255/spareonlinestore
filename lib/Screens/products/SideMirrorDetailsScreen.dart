...
import 'sidemirrordetailsinside.dart';
...
class _SideMirrorDetailsScreenState extends State<SideMirrorDetailsScreen> {
  ...
  Future<void> _loadToken() async {
    ...
    if (_token != null) {
      print('Token loaded in SideMirrorDetailsScreen: $_token');
      _fetchProducts();
    } else {
      print('No token found in SideMirrorDetailsScreen');
  ...
  Future<void> _fetchProducts() async {
    try {
      final products = await ApiService.fetchSideMirrorProducts(token: _token);
      ...
      setState(() {
        _products = products.map<Map<String, dynamic>>((product) {
          ...
          return {
            'compatibility': product['compatibility']?.toString() ?? 'No Name',
  ...
  void _sortProducts(String criteria) {
    setState(() {
      if (criteria == 'Alphabetical') {
        _filteredProducts.sort((a, b) => (a['compatibility'] ?? 'No Name')
            .compareTo(b['compatibility'] ?? 'No Name'));
  ...
  void _searchProduct(String query) {
    setState(() {
      _filteredProducts = _products.where((product) {
        final name = (product['compatibility'] ?? '').toLowerCase();
                        ...
                        return GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final favoriteProduct = {
                              'name': (product['compatibility'] ?? 'No Name')
                                  .toString(),
                            ...
                            return GestureDetector(
                              onTap: () {
                                Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (context) =>
                                        SideMirrorDetailsInside(
                                      productName:
                                          product['compatibility'] ?? 'No Name',
